...
للتطوير، يشغّل Django على 8108 وNext على 3108 وعامل البريد:
...
قبل ضبط المستلمين من لوحة التحكم، تُرسل التنبيهات إلى `DEFAULT_ALERT_EMAILS` (افتراضيًا preedargon@gmail.com). ضع `EMAIL_URL` في `.env` لإرسال فعلي، مثل `smtp+tls://USER:PASSWORD@smtp.example.com:587`. دونه تُطبع رسائل التنبيه في سجل العامل. اضبط البريد المستلم من لوحة التحكم في «المحتوى والإعدادات».
...
- الإدارة: الدخول بـ JWT، نظرة عامة، الطلبات مع التصفية والإسناد والحالات والملاحظات والسجل، المرفقات بروابط تنزيل موقّعة، عروض الأسعار والدفع وإثباته، الخدمات والمجالات والنماذج، الرسائل، الإعدادات والأسئلة الشائعة، الفريق، مؤشر التخزين، وتنبيهات البريد مع سجل المحاولات وإعادة الإرسال.
- الملفات: حقلا ملف وصورة في نماذج الخدمات، بتحقق من المحتوى الفعلي، وتخزين خاص تحت `DATA_ROOT/private` لا يُقدَّم برابط عام.
- الصلاحيات: مدير، مشغّل، منفذ يرى المسند إليه فقط. كلها مطبقة على الخادم.

## ما لم يُنفّذ بعد

الوسائط العامة وصور الخدمات، سياسة الاحتفاظ بالمرفقات، الصفحات المخصصة والتنقل وإدارة الحالات من الواجهة، النسخ الاحتياطي، والنشر على Render. المعاينة الأصلية لهذه الشاشات محفوظة كمرجع على `/design-preview` عند ضبط `ENABLE_DESIGN_PREVIEW=1`، وبياناتها محلية في المتصفح فقط.

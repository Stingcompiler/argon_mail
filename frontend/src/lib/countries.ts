...
/** Shown first: Sudan, then the countries most customers are likely in. */
const COMMON = ['SD', 'SA', 'EG', 'AE', 'QA', 'KW', 'OM', 'BH', 'SS', 'TD', 'LY', 'ET', 'ER', 'TR', 'GB', 'US'];

export const DEFAULT_COUNTRY = 'SD';

export type Country = { iso: string; dial: string; name: string };

let cache: { common: Country[]; others: Country[] } | null = null;
...
export const dialCode = (iso: string) => DIAL[iso] || DIAL[DEFAULT_COUNTRY];

/**
 * Builds the international number from the chosen country and what the
 * customer typed. The trunk prefix (leading 0, e.g. 0912…) is dropped. If
 * the customer typed a full international number (+… or 00…), it is kept.
 */
export function composePhone(iso: string, local: string): string {
  ...
  return '+' + dialCode(iso) + digits.replace(/^0+/, '');
}

/** Client-side check mirroring apps/core/phone.py (8–15 digits in E.164). */
export function phoneProblem(full: string): string | null {
  const n = full.replace(/\D/g, '').length;
  if (!full) return 'أدخل رقم الهاتف.';
  if (n < 8 || n > 15) return 'طول رقم الهاتف غير صحيح.';
  return null;
}
